...
public class AddTeacherPanel implements ActionListener{

	private JFrame frame;
	private JPanel contentPane;
	JLabel label,labelTitle1,labelTitle2,labelA,labelB;
	JTextField textField,textA, textB, textC;
	MyButtonSolid open;
	String fileName;
	String dir;
	...
	public AddTeacherPanel(){
		...
		btnYes.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				try {
					Teacher.readFile(textField.getText());
					frame.dispose();
		...
		lblName.setBounds(80, 41, 90, 16);
		panel_1.add(lblName);

		textA = new JTextField();
		textA.setBounds(208, 36, 130, 26);
		panel_1.add(textA);
		textA.setColumns(10);

		JLabel lblAge = new JLabel("Age");
		lblAge.setBounds(80, 83, 90, 16);
		panel_1.add(lblAge);

		textB = new JTextField();
		textB.setColumns(10);
		textB.setBounds(208, 78, 130, 26);
		panel_1.add(textB);
		
		JLabel lblGender = new JLabel("Gender");
		lblGender.setBounds(80, 121, 90, 16);
		panel_1.add(lblGender);

		textC = new JTextField();
		textC.setColumns(10);
		textC.setBounds(208, 116, 130, 26);
		panel_1.add(textC);
	...
	public void actionPerformed (ActionEvent event) {
		...
		{
			String teacherName = textA.getText();
			
			String gender = textC.getText();
			//check if exist
			try {
				int age = Integer.parseInt(textB.getText());
